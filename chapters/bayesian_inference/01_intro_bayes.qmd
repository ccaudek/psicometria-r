...
**Esempio 1: Misurare l'Ansia.** Quando si misura l'ansia tramite questionari, l'incertezza può derivare da:
...
**Esempio 2: Motivazione e Rinforzo Negativo.** In uno studio sull'effetto del rinforzo negativo sulla motivazione, la "motivazione interna" è una variabile latente non osservabile direttamente. Possiamo inferirla attraverso:
...
**Esempio: Inferenza sul Livello di Depressione.** In uno studio clinico sulla depressione, possiamo utilizzare l'inferenza bayesiana per stimare il livello di depressione di un paziente partendo da una distribuzione a priori informata da studi precedenti. Ogni nuovo dato raccolto (come punteggi a questionari o osservazioni) permette di aggiornare questa stima, affinando progressivamente la comprensione del fenomeno.
...
**Esempio Intuitivo: Il Globo Terrestre.** Un esempio intuitivo per spiegare l'aggiornamento bayesiano è quello proposto da @McElreath_rethinking. Supponiamo di voler stimare la proporzione della superficie terrestre coperta d'acqua. L'esperimento consiste nel lanciare un globo terrestre in aria, afferrarlo e osservare se la superficie sotto il dito è acqua o terra. Dopo ogni osservazione, possiamo aggiornare le nostre credenze sulla proporzione d'acqua (p).
...
::: {.callout-note title="Approfondimento"}
