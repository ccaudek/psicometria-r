...
Utilizziamo una camminata casuale con distribuzione normale centrata sullo stato corrente:
...
Esecuzione dell'algoritmo per campionare dalla posteriori:
...
**Interpretazione intuitiva:** Immaginate di esplorare un paesaggio montuoso (la posteriori) avvolto dalla nebbia, dove le alture rappresentano regioni ad alta densità di probabilità. L'algoritmo replica il comportamento di un escursionista che:
...
::: callout-note
## Meccanismo Chiave dell'Algoritmo

1. **Esplorazione guidata**: La proposta normale bilancia esplorazione (ampiezza passo) ed exploitation (frequenza accettazione).  
2. **Vincoli di dominio**: Le proposte al di fuori di [0,1] vengono automaticamente scartate, mantenendo $\theta$ come probabilità valida.  
3. **Efficienza computazionale**: Il calcolo evita la normalizzazione esplicita della posteriori, utilizzando il rapporto delle densità.  
4. **Memoria della catena**: Ogni stato viene registrato indipendentemente dall'accettazione, garantendo la proprietà markoviana.  
:::
...
```{r}
plot_data_post_burnin <- data.frame(Theta = post_burnin_samples)

ggplot(plot_data_post_burnin, aes(x = Theta)) +
