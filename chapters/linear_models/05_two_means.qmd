...
Nel capitolo precedente (@sec-two-groups-comparison), abbiamo analizzato la differenza tra le medie di due campioni indipendenti adottando un approccio bayesiano. In quel contesto, i due gruppi erano considerati entità separate, e la differenza tra le medie è stata calcolata direttamente.

Un'alternativa a questo approccio consiste nell’utilizzo di un modello di regressione. In questo caso, anziché calcolare direttamente la differenza tra le medie, si introduce una variabile indicatrice (o "dummy") $D$ nel modello di regressione, come segue:
...
Riprendiamo il dataset relativo al quoziente di intelligenza (QI) dei bambini, distinguendo tra quelli le cui madri hanno completato la scuola superiore e quelli le cui madri non l'hanno completata. L'obiettivo è replicare i risultati precedenti utilizzando un'analisi di regressione bayesiana. Vediamo come implementare questa analisi passo per passo.
...
```r
fit_1 <- brm(kid_score ~ mom_hs, data = kidiq, prior = prior_gaussian)
```
...
Un modello di regressione con **sola intercetta** è uno strumento estremamente utile per fare inferenza sulla media della popolazione, specialmente in un contesto bayesiano. Questo modello è equivalente a stimare la media di un campione e consente di costruire intervalli di credibilità (nel caso bayesiano) o intervalli di confidenza (nel caso frequentista) per la stima della media.

### Modello di Regressione con Sola Intercetta

Quando utilizziamo un modello di regressione con sola intercetta, l'equazione del modello si semplifica a:
...
- $\mu$ è l'intercetta, che rappresenta la media stimata della popolazione,
- $\varepsilon_i$ è l'errore casuale, solitamente distribuito come $\mathcal{N}(0, \sigma^2)$.

### Inferenza sulla Media

Nel contesto bayesiano, il modello consente di stimare $\mu$ e $\sigma$ sulla base della distribuzione a posteriori, ottenuta combinando i dati osservati con le distribuzioni a priori. Questo approccio permette di calcolare:

- **L'intervallo di credibilità**: un intervallo entro cui si trova la media della popolazione con una certa probabilità (es., 89% o 95%).
- **La distribuzione a posteriori di $\mu$**: che fornisce un quadro completo dell'incertezza associata alla stima della media.

Nel contesto frequentista, un modello equivalente calcola la media campionaria e costruisce un **intervallo di confidenza** basato su ipotesi di normalità e su una stima puntuale della varianza campionaria.

### Confronto con il Test $t$ di Student

Il modello di regressione con sola intercetta è concettualmente analogo al **test $t$ di Student** per una singola media. Nel caso del test $t$, l'ipotesi nulla è che la media della popolazione sia uguale a un valore specifico $\mu_0$:

$$
H_0: \mu = \mu_0.
$$

Il test calcola la statistica $t$ come:

$$
t = \frac{\bar{y} - \mu_0}{s / \sqrt{n}},
$$

dove $\bar{y}$ è la media campionaria, $s$ è la deviazione standard campionaria, e $n$ è la dimensione del campione.

Nel caso bayesiano, invece di testare un'ipotesi nulla, si calcola direttamente la probabilità a posteriori che $\mu$ sia maggiore, minore o uguale a $\mu_0$, offrendo un'inferenza più intuitiva e flessibile.

### Vantaggi del Modello Bayesiano

1. **Intervalli di Credibilità**: Forniscono una probabilità esplicita che la media della popolazione si trovi in un certo intervallo, mentre gli intervalli di confidenza frequentisti si basano su ripetizioni ipotetiche del campionamento.
   
2. **Incorporazione di Priori**: Il modello bayesiano consente di integrare conoscenze pregresse (attraverso i priori), il che può essere particolarmente utile in contesti con campioni piccoli o conoscenze preesistenti.

3. **Inferenza Diretta**: Piuttosto che concentrarsi su una statistica test, l'approccio bayesiano fornisce una distribuzione a posteriori che descrive completamente l'incertezza sulla media.

In conclusione, il modello di regressione con sola intercetta rappresenta un'alternativa elegante e versatile al test $t$ per una singola media, sia in un contesto frequentista che bayesiano. La flessibilità del metodo bayesiano lo rende particolarmente adatto per analisi che richiedono una rappresentazione più completa e probabilistica dell'incertezza.
