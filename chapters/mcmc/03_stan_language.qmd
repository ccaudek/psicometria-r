...
Per analizzare i campioni generati, possiamo utilizzare istogrammi e tracce MCMC. In R, possiamo creare un istogramma della distribuzione a posteriori di $\theta$. 

Convertiamo i campioni in un vettore:
...
Creiamo un istogramma della distribuzione a posteriori di $\theta$:
...
Questo esempio dimostra come utilizzare prior debolmente informativi per modellare dati binomiali in Stan. Si noti come la stima putuale a posteriori e l'intervallo di credibilità riproducono i valori ottenuti utilizzando l'algoritmo di Metropolis (@sec-mcmc-metropolis).

## Diagnostiche di Campionamento

Per valutare la qualità del campionamento, possiamo utilizzare la statistica $\widehat{R}$ e il numero effettivo di campioni ($N_{\text{eff}}$):

```{r}
fit2$summary()
```

Un valore di $\widehat{R}$ vicino a 1 indica che le catene sono convergenti, mentre un $N_{\text{eff}}$ elevato suggerisce una stima precisa dei parametri.
...
È anche possibile usare la funzione `diagnostic_summary()`:
...
I risultati ottenuti mostrano che il campionamento è stato efficace:
...
In sintesi, Stan, grazie alla sua implementazione dell'algoritmo NUTS, offre un framework potente e flessibile per l'inferenza bayesiana. Attraverso una combinazione di modelli probabilistici ben strutturati, diagnostica avanzata e strumenti di visualizzazione, è possibile affrontare con successo problemi complessi, dalle simulazioni in avanti alle stime inverse dei parametri.
...
Lo stesso risultato può essere ottenuto con le funzioni di **bayesplot**:
  
```{r}
mcmc_hist(fit3$draws("theta"))
```
...
I quantili forniscono una descrizione più completa della distribuzione a posteriori. Un intervallo di credibilità è un intervallo di quantili che cattura una proporzione specificata della massa di probabilità. Ad esempio, un intervallo di credibilità al 90% è dato dai quantili al 5% e al 95%. In R, possiamo calcolarli così:
...
Questi valori rappresentano un intervallo che contiene il 90% della distribuzione a posteriori di $\theta$, con il 5% al di sotto e al di sopra di esso.
...
### Bias e Errore di Stima

L'errore di una stima è definito come la differenza tra il valore stimato e quello vero:

$$
\text{err} = \hat{\theta} - \theta.
$$

Il bias di uno stimatore è l'errore atteso rispetto alla distribuzione dei dati:

$$
\text{bias} = \mathbb{E}[\hat{\theta}(Y) - \theta].
$$

Un bias nullo indica che lo stimatore è corretto in media, cioè non tende a sovrastimare o sottostimare sistematicamente il valore vero.
