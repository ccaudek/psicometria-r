...
<span class="sottopasso">1. Estraiamo i campioni di $\lambda$</span>
...
<span class="sottopasso">2. Calcoliamo i parametri della distribuzione posteriore teorica<span>
...
<span class="sottopasso">3. Confrontiamo i due risultati (MCMC vs formula)<span>
...
<span class="sottopasso">1. Calcoliamo i quantili della distribuzione<span>
...
<span class="sottopasso">2. Interpretazione dei risultati<span>
...
ggplot(data.frame(x = x_draws), aes(x = x)) +
  geom_histogram(
    bins = 30, color = "white", fill = rinascimentale_primaria["seppia_scuro"],
    alpha = 0.5
    ) +
  labs(
    x = "Tasso (eventi/anno)",
