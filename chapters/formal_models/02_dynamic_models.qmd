...
Seguendo la discussione di @knight2023tutorial, in questo capitolo estenderemo il modello di gruppo esaminato nel capitolo precedente in modo da esaminare:

1. il modello a livello individuale, nel quale si stima un $\alpha$ e un $\beta$ per ogni partecipante;
2. il modello gerarchico (multilevel), nel quale i parametri individuali sono estratti da una distribuzione comune (ad esempio, una distribuzione normale);
3. il modello per gruppi noti, in cui si confrontano i parametri tra le condizioni sperimentali (ad esempio, il gruppo "approach" vs. il gruppo "avoidance").

## Person-Level Model

Il Sample-Level Model descrive il processo di aggiornamento degli obiettivi a livello di gruppo, senza tenere in considerazione le differenze individuali. Tuttavia, è possibile estendere il modello descritto nel @sec-dynamic-models-goal-updating in modo tale da stimare i parametri $\alpha$ e $\beta$ per ciascun partecipante.
...
In questo capitolo abbiamo introdotto e confrontato tre modelli bayesiani progressivamente più complessi per descrivere il processo di aggiornamento degli obiettivi nel tempo.

1) un modello a livello individuale, in cui ogni partecipante ha i propri parametri stimati separatamente;
2) un modello gerarchico (multilevel), in cui i parametri individuali sono vincolati da distribuzioni di gruppo, favorendo stime più stabili attraverso lo shrinkage;
3) un modello gerarchico con gruppi noti, in cui le distribuzioni dei parametri variano a seconda della condizione sperimentale (es. "approach" vs. "avoidance"), che permette di testare le differenze tra i gruppi a livello dei parametri latenti.

L’analisi ha chiaramente mostrato come l’introduzione di una struttura gerarchica migliori sensibilmente la qualità predittiva del modello (come indicato dai valori di ELPD), rispetto all’approccio che stima i parametri separatamente per ciascun individuo. Il modello gerarchico sfrutta la somiglianza tra i soggetti per regolarizzare le stime e ottenere parametri più affidabili, soprattutto in presenza di pochi dati per soggetto, condizione tipica di molti studi psicologici.

Il modello con gruppi noti ha permesso di esplorare l'ipotesi che ci siano differenze tra le condizioni nel tasso di apprendimento (`alpha`) e nella coerenza decisionale (`beta`). Sebbene gli intervalli di credibilità al 95% includano lo zero, le probabilità a posteriori superiori al 96% indicano un supporto consistente a favore di un effetto negativo delle condizioni sulla media dei parametri. Questo esempio illustra il vantaggio della prospettiva bayesiana: anche in assenza di una "significatività convenzionale", è possibile quantificare il grado di supporto per ipotesi direzionali e valutare la rilevanza pratica delle stime ottenute.

Dal punto di vista modellistico, il confronto tra i tre approcci evidenzia l'importanza di una modellazione gerarchica e l'utilità di includere informazioni sperimentali (come l'appartenenza a gruppi) nel livello degli iperparametri. In molti contesti psicologici, infatti, i partecipanti non sono perfettamente indipendenti, ma condividono caratteristiche biologiche, cognitive e contestuali che giustificano una struttura a più livelli nei modelli inferenziali.

In sintesi, l'approccio bayesiano multilevel offre un quadro flessibile e potente per modellare processi dinamici caratterizzati da variabilità interindividuale e tra gruppi. Le tecniche presentate in questo capitolo forniscono una solida base per affrontare scenari più complessi, come quelli che coinvolgono i modelli di miscela, relativi a classificazioni non note, che saranno esplorati nei capitoli successivi.
