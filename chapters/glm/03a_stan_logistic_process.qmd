...
##  Motivazione: perché oltre il GLM?
...
## Regressione logistica: il ponte intuitivo
...
### Oltre il GLM: un modello processuale semplice
...
## Stan: modello processuale (AR(1) sulla scala del logit)
...
## Preprocess e stima in `cmdstanr`
...
fit$summary() %>%
  dplyr::filter(
    grepl("alpha_mu|alpha_sigma|beta|phi|sigma_u", variable)
  )
```
...
## Confronto (rapido) con GLM logit classico
...
## Estensione gerarchica (varianza tra soggetti)
...
## Prior Predictive Check (prima dei dati!)
...
## Cosa abbiamo guadagnato rispetto al GLM
...
##  Appendice: Stan completo (versione gerarchica su $\beta$)
...
'
stan_file <- write_stan_file(stan_hier)
stan_file
```


```{r}
mod_hier <- cmdstan_model(stan_file)
```
...
```{r}
# Rhat e ESS sintetici
fit_hier$summary(c("alpha_mu","alpha_sigma","beta","phi","sigma_u"))
```
