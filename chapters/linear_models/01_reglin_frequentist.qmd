...
* **esplorare relazioni** tra costrutti psicologici,
...
1. **Regressione bivariata** – Un solo predittore e un solo esito.
2. **Regressione multipla** – Un solo esito e molteplici predittori.
3. **Regressione multivariata** – Più esiti simultaneamente, con uno o più predittori.
...
### Stima manuale dei coefficienti
...
Ho individuato una sezione che può essere resa più chiara e compatta:
quella in cui spieghi il **metodo dei minimi quadrati** e le **formule di stima di a e b**.
Attualmente il testo ripete più volte le stesse definizioni (es. passaggio per il baricentro, minimizzazione SSE) e alterna spiegazioni concettuali e procedurali senza una sequenza ben definita.


## Metodo dei minimi quadrati: principi e formule
...
### Simulazione in R: distribuzione campionaria di $\hat{\beta}_1$
...
1. **Non distorsione**
   La media empirica è ≈ 1.5 → conferma $E(\hat{\beta}_1) = \beta_1$ sotto le assunzioni di Gauss–Markov.

2. **Precisione**
   La deviazione standard empirica è molto vicina all’errore standard teorico
   $SE(\hat{\beta}_1) = \sqrt{\sigma^2 / \sum (X_i - \bar{X})^2}$.

3. **Forma della distribuzione**
   L’istogramma è ben approssimato da una curva normale → normalità asintotica confermata anche per $n = 30$.
