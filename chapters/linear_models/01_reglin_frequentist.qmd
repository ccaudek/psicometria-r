...
Quando studiamo un fenomeno psicologico, spesso ci chiediamo: *esiste una relazione tra due variabili?* Ad esempio:  

- *L’ansia influisce sul rendimento accademico?*  
- *Il tempo di studio è associato a risultati migliori nei test?*  

Un **modello statistico** è una rappresentazione semplificata della realtà che ci aiuta a rispondere a domande come queste. In particolare, ci dice cosa succede, **in media**, a una variabile (che chiamiamo *Y*, o **variabile dipendente**) quando un’altra variabile (*X*, o **variabile indipendente**) cambia.  

### La relazione tra $X$ e $Y$ 

Il modello di **regressione lineare semplice** è il più semplice strumento per descrivere una relazione tra due variabili. L’idea di base è che, se *X* e *Y* sono associate, possiamo rappresentare questa relazione con una **retta**, che esprime come *Y* tende a variare al variare di *X*.  
...
Geometricamente, questa equazione corrisponde a una **retta di regressione**, che rappresenta la migliore approssimazione lineare della relazione tra *X* e *Y* nei dati osservati.  La sua semplicità lo rende un punto di partenza ideale per comprendere la logica della modellazione statistica.

### Perché "in media"?  

Nella realtà, raramente tutti i punti dati giacciono perfettamente sulla retta: ci sono sempre variazioni individuali. La regressione lineare non pretende di prevedere *esattamente* il valore di *Y* per ogni individuo, ma ci dice cosa ci aspettiamo **in media** in una popolazione.  
...
**Avvertenza cruciale:** in tutti questi contesti, è **essenziale** che il modello includa tutte le variabili rilevanti per la variabile di interesse. Ad esempio, in uno studio sull’efficacia di una psicoterapia per la depressione, fattori come età, condizioni di salute preesistenti e supporto sociale possono influenzare significativamente i risultati e devono essere integrati nell’analisi. L'omissione di tali variabili può distorcere le relazioni stimate tra predittori e risultato -- un problema noto come **errore di specificazione**. L’accuratezza dei modelli di regressione dipende quindi sia dalla completezza del modello.  

### Stima dei parametri: due prospettive a confronto

#### L’approccio frequentista (minimi quadrati / massima verosimiglianza)

Nel quadro tradizionale si individuano i coefficienti $a$ e $b$ cercando la retta che minimizza la somma degli errori quadratici, $\sum e_i^2$, con $e_i = y_i - (a + bx_i)$. Posto che gli errori seguano una distribuzione normale $N(0,\sigma^2)$, la procedura di minimi quadrati coincide con la massima verosimiglianza, producendo stime puntuali univoche: in pratica otteniamo “la” migliore retta per i dati osservati. L’incertezza viene quantificata tramite l’intervallo di confidenza al 95 %; ciò significa che, se ripetessimo l’esperimento molte volte, nel 95 % delle repliche la retta vera cadrebbe dentro quello stesso intervallo. La forza di questa impostazione sta nella sua semplicità operativa, ma la sua interpretazione probabilistica rimane ancorata a una logica di frequenze a lungo termine e non integra informazioni pregresse.

#### L’approccio bayesiano

La regressione bayesiana amplia il quadro introducendo distribuzioni **a priori** sui parametri. In pratica si specifica una credenza iniziale—forse vaga, forse basata su studi precedenti—per $a$, $b$ e $\sigma$, e la si aggiorna con i dati attraverso il teorema di Bayes:

$$
P(a,b,\sigma \mid \text{dati}) \propto P(\text{dati}\mid a,b,\sigma)\,P(a,b,\sigma).
$$

Il risultato non è più una singola retta, bensì una distribuzione di credibilità che assegna, ad esempio, il 95 % di probabilità al fatto che il coefficiente $b$ cada fra 0.8 e 1.2. Questo schema consente di incorporare evidenze esterne (una meta-analisi, esperimenti precedenti, teoria) e si rivela particolarmente robusto con campioni piccoli: un *prior* informativo—per esempio $b \sim N(-0.7,0.3)$ se la letteratura suggerisce che l’ansia riduca la performance—verrà temperato o rafforzato dai nuovi dati, mentre un *prior* poco informativo, come $b \sim N(0, 100)$, lascerà sostanzialmente campo libero alle evidenze empiriche, avvicinando l’esito a quello frequentista.

#### Quale scegliere?

Con campioni ampi e obiettivi standard, l’inferenza frequentista resta una scelta pratica e largamente accettata. Il paradigma bayesiano diventa preferibile quando si dispone di conoscenza pregressa rilevante, si desidera un’interpretazione probabilistica diretta dei parametri o ci si trova a lavorare con dati limitati. Così, un ricercatore che replichi un’analisi nota della relazione fra terapia cognitiva ($X$) e ansia ($Y$) può affidarsi a minimi quadrati, mentre chi intenda sintetizzare studi correlati in un’unica analisi aggiornata troverà più naturale la via bayesiana.
