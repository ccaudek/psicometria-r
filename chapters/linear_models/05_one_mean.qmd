...
### Intervallo di confidenza al 95% per $\alpha$
...
Questo intervallo si basa sulla seguente formula classica:
...
* `coef(fm1)` restituisce la stima di $\alpha$,
* `qt(0.975, df.residual(fm1))` restituisce il valore critico della distribuzione $t$ per un intervallo al 95% (con 2,5% di probabilità in ciascuna coda),
* `0.413` è l’errore standard associato a $\hat{\alpha}$.
