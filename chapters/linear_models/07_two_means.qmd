...
- Le le basi concettuali e statistiche che sottendono la modellazione della differenza tra medie nell'ambito del modello di regressione lineare bayesiana.
...
::: {.callout-important collapse=true title="Obiettivi di apprendimento"}
...
ggplot(kidiq, aes(x = mom_hs, y = kid_score)) +
  ...
  labs(
    ...
    y = "QI del bambino",
    title = "Distribuzione dei punteggi QI per livello di istruzione materna"
  )
```
...
* Un modello è “meglio” se ha *ELPD* maggiore.
* Se la differenza di ELPD è *simile* al suo errore standard (*se_diff*), il vantaggio è *modesto*: in questi casi preferisci il *più semplice* che passa i PPC.
* Controlla i *Pareto $k$*: se molti $k>0.7$, usa `reloo=TRUE` o k-fold.

Nel caso presente:

* “Il confronto LOO indica un lieve vantaggio predittivo della likelihood *skew-normal* rispetto alle alternative gaussiane (omoscedastica/eteroscedastica) e alla *t*. Tuttavia, le differenze di ELPD sono dell’ordine dell’errore standard, per cui l’evidenza a favore della skew-normal è *moderata*.”
* “Le *posterior predictive checks* mostrano un miglior allineamento delle code/assimmetria con la skew-normal; per questo motivo la adottiamo come *modello principale*, riportando in parallelo la *sensibilità* delle stime di $\Delta$ alle specifiche di likelihood.”
* “Il leggero vantaggio del modello eteroscedastico rispetto all’omoscedastico suggerisce *differenze di variabilità* tra gruppi, ma l’impatto predittivo è *contenuto*.”

In sintesi,

* L’ELPD favorisce la *skew-normal* ma con *differenze piccole* rispetto alle alternative.
* La scelta del modello va giustificata *congiuntamente* da (i) compatibilità predittiva (LOO), (ii) *PPC* e (iii) *robustezza* delle quantità sostantive $\Delta$, $\Pr(\Delta>0)$, $\Pr(|\Delta|>\text{SESOI})$.
* Quando le differenze predittive sono deboli, la *stabilità* di $\Delta$ tra modelli è la chiave per l’interpretazione sostantiva.

In sintesi, questi passaggi mantengono l’analisi *trasparente* e *riproducibile*, e collegano le stime a domande pratiche (via SESOI) senza introdurre soglie arbitrarie.
...
## Per concludere: oltre la media del gruppo {.unnumbered .unlisted}
