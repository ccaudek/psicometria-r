...
* Su dati generati da un **AR(3)**, il modello **AR(3)**:

  * recupera coefficienti coerenti con la verità,
  * mostra buona aderenza visiva nei PPC,
  * **vince** su **AR(1)** in termini di **ELPD-LOO**.
* Didatticamente, il confronto AR(3) vs AR(1) illustra che:

  * modelli troppo semplici (AR(1)) possono sottocatturare la struttura temporale,
  * **LOO** fornisce un criterio pratico per la scelta del modello basata sulla **predizione**.

### Estensioni suggerite

* Aggiungere regressori esogeni (ARX): p.es., *stress del giorno* → umore.
* Passare a **modelli gerarchici** per **più partecipanti** (EMA multiserie).
* Usare **Student-t** per robustezza agli outlier.
* Confrontare con **GP** o **HMM** quando si sospettano trend non lineari o salti di stato.
...
Se vuoi, preparo subito una **variante gerarchica** (multi-partecipante) e una **versione robusta Student-t**, mantenendo lo stesso flusso: simulazione → fit → PPC → LOO.
```
