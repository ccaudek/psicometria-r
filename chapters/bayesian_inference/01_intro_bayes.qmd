...
# Beta function
beta_function <- function(W, L, p) {
  return(factorial(W + L + 1) / (factorial(W) * factorial(L)) * p^W * (1 - p)^L)
}

# Function to plot the beta distribution from observations
plot_beta_from_observations <- function(observations, resolution = 50, ...) {
  n_W <- sum(strsplit(observations, NULL)[[1]] == "W")
  n_L <- nchar(observations) - n_W
  proportions <- seq(0, 1, length.out = resolution)
  
  probs <- sapply(proportions, function(p) beta_function(n_W, n_L, p))
  
  plot(proportions, probs, type = "l", ylab = "", xlab = "", ...)
  title(observations)
}
...
for (ii in 1:9) {
  ...
  plot.new()
  plot.window(xlim = c(0, 1), ylim = c(0, max(beta_function(0, 0, seq(0, 1, length.out = 50)))))
  
  # Plot previous observations (if any)
  if (ii > 1) {
    plot_beta_from_observations(substr(observations, 1, ii - 1), col = "black", lty = 2)
  } else {
    plot_beta_from_observations("", col = "black", lty = 2)
  }
  
  # Set the color based on the observation
  color <- ifelse(substr(observations, ii, ii) == "W", "red", "blue")
  
  # Plot the current observation
  plot_beta_from_observations(substr(observations, 1, ii), col = color, lwd = 4, alpha = 0.5)
  
  # Y-axis label every 3rd plot
  if (ii %% 3 == 1) {
    mtext("posterior probability", side = 2, line = 2)
  }
}
```
