...
5. Selezionare casualmente 50 persone e determinare quante mostrano una predisposizione alla creatività, misurata attraverso un questionario standardizzato.
...
Un aspetto interessante di questo esperimento è l'**andamento della frequenza relativa di Testa** al crescere del numero di lanci. Il grafico seguente mostra l’evoluzione della **media cumulativa** delle Teste, che dovrebbe convergere al valore teorico di **0.5**.
...
- Con quale rapidità la frequenza cumulativa delle Teste converge al valore teorico $p = 0.5$?
...
I modelli matematici usati per descrivere un esperimento casuale sono costituiti da tre componenti principali:

1. **lo spazio campionario**,  
2. **gli eventi**,  
3. **la probabilità**.

Esaminiamo di seguito ciascuna di queste componenti.
...
Una *probabilità* $P$ è una funzione $P: \mathcal{F} \to [0,1]$ che assegna a ogni evento $A$ un numero reale compreso tra 0 e 1, soddisfando i seguenti assiomi:

1. **Non-negatività e normalizzazione**:  
   $0 \leq P(A) \leq 1$ per ogni evento $A \subseteq \Omega$.

2. **Evento certo**:  
   $P(\Omega) = 1$.

3. **Additività numerabile**:  
   Se $A_1, A_2, \dots$ sono eventi **mutualmente esclusivi** ($A_i \cap A_j = \emptyset$ per $i \neq j$), allora:  
   $$
   P\left(\bigcup_{i=1}^\infty A_i\right) = \sum_{i=1}^\infty P(A_i) .
   $$ 
:::

### Interpretazione degli Assiomi
- **Assioma 1**: La probabilità è sempre una misura compresa tra 0 (evento impossibile) e 1 (evento certo).  
  *Esempio*: Nel lancio di un dado, $P(\text{"Esce 7"}) = 0$.

- **Assioma 2**: Lo spazio campionario $\Omega$ rappresenta tutti i possibili risultati di un esperimento. Per definizione, almeno uno di questi esiti deve verificarsi. Pertanto, la probabilità associata a $\Omega$ è sempre massima e uguale a 1.  
  *Esempio*: $P(\text{"Esce un numero tra 1 e 6"}) = 1$.

- **Assioma 3**: La probabilità dell'unione di eventi incompatibili è la somma delle singole probabilità.  
  *Esempio*: Se $A = \text{"Esce pari"}$ e $B = \text{"Esce dispari"}$, allora $P(A \cup B) = P(A) + P(B)$.

### Proprietà Fondamentali {#subsec-proprieta}

Dagli assiomi precedenti (assiomi di Kolmogorov) derivano immediatamente le seguenti proprietà:

::: {.theorem #thm-proprieta-probabilita}
Siano $A$ e $B$ eventi qualsiasi in $\Omega$. Valgono:
...
::: {#exm-}  
**Misurazione dell'altezza** degli uomini adulti, modellata come variabile aleatoria continua $X$ (in cm) con distribuzione normale (si veda il @sec-prob-cont-prob-distr): 

- **Media**: $\mu = 170$ cm  
- **Deviazione standard**: $\sigma = 7$ cm  

La funzione di densità (PDF) è: 

$$
f(x) = \frac{1}{7\sqrt{2\pi}} e^{-\frac{(x - 170)^2}{2 \cdot 7^2}} \quad \left(\mathcal{N}(170, 7^2)\right).
$$
...
La probabilità corrisponde all’area sotto la curva della PDF tra 160 e 180 cm:  
$$
P(A) = \int_{160}^{180} \frac{1}{7\sqrt{2\pi}} e^{-\frac{(x - 170)^2}{98}}\, dx.
$$  
...
$$
Vedremo in seguito come questa probabilità possa essere facilmente calcolata usando R nel modo seguente:
...
| **Probabilità di un singolo punto** | $P(\{\omega_i\}) = p_i$ (≥ 0)       | $P(\{x\}) = 0$ (sempre zero)        |  
...
::: {#exm-}
Supponiamo di avere un'urna con 10 palline numerate da 1 a 10 e di voler calcolare la probabilità di estrarre una pallina con un numero pari:
...
Per problemi più complessi, come il calcolo della probabilità di ottenere una determinata combinazione di carte o di formare gruppi specifici da una popolazione, utilizziamo tecniche combinatorie più avanzate, come permutazioni e combinazioni (si veda l'@sec-apx-combinatorics).
