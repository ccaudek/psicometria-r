...
::: {.epigraph}
> “The gamma distribution is conjugate to the Poisson likelihood for the rate parameter, so that the posterior distribution is also gamma. This makes the Gamma–Poisson model a natural choice for Bayesian analysis of count data.”
>
...
Il parametro centrale della distribuzione di Poisson è $\lambda$, che rappresenta il *numero medio di eventi per unità di misura* (ad esempio, la media di compulsioni per ora, oppure la media di parole per frase). Stimare questo parametro significa, in pratica, descrivere l’intensità con cui l’evento che ci interessa tende a verificarsi.
...
Questo doppio percorso ci consente di verificare che i metodi numerici funzionano correttamente in un caso semplice, dove conosciamo già la soluzione analitica, e ci prepara ad affrontare situazioni più complesse, in cui la soluzione chiusa non esiste e le procedure basate sulla simulazione numerica diventano indispensabili.
...
ggplot(df_long, aes(x = lambda, y = Densita, color = Distribuzione)) +
  geom_line(size = 1) +
  labs(title = "Aggiornamento bayesiano con il metodo a griglia",
       x = expression(lambda),
       y = "Densità")
```
...
In questo capitolo sono stati presentati i modelli per dati di conteggio basati sulla distribuzione di Poisson e sulla coppia coniugata Gamma–Poisson. L'utilizzo di una distribuzione a priori Gamma per il tasso $\lambda$ consente un aggiornamento analitico della credenza: la distribuzione a posteriori rimane di tipo Gamma e la sua media risulta essere una media pesata tra la media a priori e la media campionaria. Ciò rende esplicito il fenomeno di *shrinkage*: con pochi dati, le stime tendono verso la credenza iniziale; all'aumentare di $n$, prevale invece l'informazione contenuta nei dati.

L'approssimazione numerica su griglia ha permesso di illustrare lo stesso processo di aggiornamento in forma discreta, offrendo un utile supporto didattico per visualizzare il contributo relativo della prior e della verosimiglianza. In contesti più complessi — quali modelli con predittori multipli, strutture gerarchiche o dati longitudinali — la stessa logica inferenziale può essere estesa mediante tecniche di campionamento MCMC (ad esempio con Stan), mantenendo la validazione del modello attraverso *posterior predictive checks*.

Dal punto di vista applicativo, il modello di Poisson è appropriato quando:

- i dati sono conteggi interi non negativi;
- le osservazioni sono condizionatamente indipendenti dato $\lambda$;
- media e varianza dei conteggi sono di ordine di grandezza comparabile.

In presenza di esposizioni differenziate tra le unità (ad esempio durate, aree o numero di prove differenti), è necessario introdurre un *offset* per modellare correttamente il tasso sottostante. Qualora la varianza osservata ecceda sistematicamente la media (*over-dispersione*), è opportuno considerare modelli più flessibili, come la Poisson–Gamma (Binomiale Negativa) con parametro di dispersione, modelli a inflazione di zeri o strutture gerarchiche che catturino l’eterogeneità non osservata.

Le verifiche predittive dovrebbero confrontare i dati osservati con repliche simulate dalla posterior sotto diversi aspetti: distribuzione dei conteggi (inclusa la frequenza di zeri), media, varianza, presenza di code pesanti o valori estremi. Scostamenti sistematici possono indicare violazioni delle assunzioni di base, quali over-dispersione, eterogeneità non modellata o dipendenze temporali.

Per una rendicontazione completa, si raccomanda di includere:

- la specifica formale del modello (inclusi offset e predittori, se presenti);
- la scelta della prior, in forma parametrica e con relativa giustificazione;
- sintesi della posterior di $\lambda$ (mediana, intervallo di credibilità al 95%, eventuali confronti con valori di riferimento);
- risultati dei *posterior predictive checks*, con commento degli eventuali scostamenti;
- indicazioni su limiti emersi (es. segnali di over-dispersione residua) e possibili estensioni del modello.
