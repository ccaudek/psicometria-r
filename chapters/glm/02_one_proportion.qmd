...
In questo capitolo abbiamo esaminato il modello binomiale per lo studio delle proporzioni, associato alla distribuzione Beta come prior coniugata. Questa scelta si rivela particolarmente vantaggiosa grazie alla sua capacità di semplificare i calcoli: la distribuzione a posteriori mantiene infatti la forma Beta, permettendo di derivare agevolmente medie, varianze, intervalli credibili e distribuzioni predittive per osservazioni future. In altri termini, è possibile osservare in modo trasparente come le informazioni introdotte dalla prior si integrino con quelle provenienti dai dati.

### Elementi essenziali per la comunicazione dei risultati

Nella comunicazione di un'analisi basata su una proporzione, è opportuno includere tre elementi fondamentali. Innanzitutto, è cruciale presentare una stima della proporzione, espressa attraverso la media o la mediana della distribuzione a posteriori, corredata da un intervallo credibile che ne quantifichi l'incertezza. In secondo luogo, risulta utile calcolare e riportare la probabilità che la proporzione superi una determinata soglia di interesse clinico o pratico, oppure che ricada all'interno di un intervallo di equivalenza pratica (ROPE). Infine, la distribuzione predittiva a posteriori consente di anticipare i possibili esiti in campioni futuri, rispondendo in modo diretto alla domanda su cosa ci si possa aspettare in osservazioni successive. Questi tre aspetti consentono di superare la logica dicotomica del valore-$p$, offrendo risposte più ricche e pertinenti alle domande della ricerca applicata.


### Scelta e interpretazione della prior

La distribuzione Beta offre una notevole flessibilità nella specificazione della prior, potendo essere calibrata in base al contesto e alle conoscenze preesistenti. La $\mathcal{Beta}(1, 1)$, per esempio, corrisponde a una distribuzione uniforme e rappresenta un approccio deliberatamente non informativo. La $\mathcal{Beta}(2, 2)$, concentrando la massa di probabilità attorno al valore 0.5, risulta appropriata quando si prevede una proporzione centrale. Al contrario, la $\mathcal{Beta}(0.5, 0.5)$ attribuisce maggiore probabilità ai valori estremi, vicini cioè a 0 o 1, ed è quindi indicata in contesti in cui sono plausibili eventi rari o frequenze molto elevate. Qualora siano disponibili dati storici o evidenze precedenti, è possibile determinare i parametri $a$ e $b$ in modo da rispecchiare una stima precedente della proporzione e il grado di certezza associato.

### Sostegno alle decisioni attraverso le probabilità

L'approccio bayesiano consente di fondare le decisioni su valutazioni probabilistiche direttamente legate agli interrogativi della ricerca. È così possibile stimare, in termini di probabilità, la superiorità di una proporzione rispetto a una soglia di riferimento, la sua equivalenza pratica entro un intervallo predefinito o la sua non-inferiorità rispetto a un margine clinico. Tali probabilità, essendo espresse in modo continuo e intuitivo, forniscono un supporto decisionale più informativo e sfumato rispetto al dualismo insito nel rigetto o nell'accettazione dell'ipotesi nulla.

### Verifica dell’adeguatezza del modello

Il modello binomiale poggia sull'assunzione di indipendenza tra le osservazioni e di costanza della probabilità di successo. Qualora i dati manifestino una variabilità superiore a quanto atteso (sovradispersione) o provengano da popolazioni eterogenee, è necessario considerare estensioni del modello, come quello Beta-Binomiale o modelli a struttura gerarchica. I *posterior predictive check* costituiscono uno strumento fondamentale per valutare se le previsioni del modello sono in linea con i dati osservati, offrendo così un riscontro pratico sulla bontà dell'adattamento.

### Progettazione degli studi e assicurazione bayesiana

Un ulteriore vantaggio dell’impostazione bayesiana emerge in fase di progettazione dello studio. Attraverso apposite simulazioni, è possibile stimare la probabilità che, a partire da un determinato campione, si riesca a ottenere una conclusione rilevante, ad esempio che la proporzione superi una data soglia con un determinato livello di credibilità. Questa probabilità, nota come *assurance bayesiana*[^1], rappresenta un'alternativa più realistica al concetto di potenza statistica di stampo frequentista, in quanto incorpora esplicitamente l'incertezza sui parametri prima ancora della raccolta dei dati.

[^1]: L'*assurance bayesiana* rappresenta la probabilità che, una volta raccolti i dati, l'analisi finale porti a una determinata conclusione decisionale. Per esempio, potremmo voler calcolare la probabilità che il nostro studio futuro concluda che "la proporzione supera 0.5 con almeno il 95% di credibilità". Il calcolo dell'assurance avviene attraverso un processo di simulazione in tre fasi. Si inizia generando un gran numero di possibili campioni di dati, ciascuno simulato considerando la nostra attuale incertezza sul parametro, così come espressa dalla distribuzione prior. Successivamente, per ogni campione simulato, si esegue l'analisi bayesiana completa, ottenendo una distribuzione a posteriori. Si verifica quindi se la conclusione desiderata (per esempio, che l'intervallo credibile al 95% sia superiore a una certa soglia) viene raggiunta. La frequenza con cui questa condizione si verifica tra tutte le simulazioni rappresenta il valore dell'assurance. Questo approccio risulta particolarmente realistico perché non presume di conoscere il valore vero del parametro, ma tiene invece conto di tutta l'incertezza rappresentata dalla nostra distribuzione prior.

In sintesi, l'approccio bayesiano applicato al confronto delle proporzioni supera la logica semplicistica del verdetto binario, consentendo invece di stimare in modo diretto le differenze, quantificare l'incertezza in termini probabilistici, formulare previsioni verificabili e supportare processi decisionali complessi. Per lo psicologo, questo si traduce nella possibilità di trarre conclusioni meglio calibrate, più informative e più saldamente ancorate alle effettive domande di ricerca.
