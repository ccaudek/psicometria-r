...
Nella pratica statistica, i parametri della popolazione sono generalmente sconosciuti, ma possiamo stimarli utilizzando le informazioni ottenute da un campione. La seguente tabella riassume i simboli più comuni utilizzati per distinguere tra quantità note e sconosciute nel contesto dell'inferenza statistica. Questa distinzione è fondamentale per comprendere cosa possiamo effettivamente calcolare dai dati e cosa rimane invece incerto.
...
Utilizzando un campione casuale di ampiezza $n$:

- La stima più naturale per la media della popolazione $\mu$ è la media campionaria $\bar{Y}$.
- La stima più naturale per la varianza della popolazione $\sigma^2$ è:

$$
\hat{\sigma}^2 = \frac{1}{n-1} \sum_{i=1}^n (Y_i - \bar{Y})^2.
$$

Queste considerazioni, tuttavia, sono puramente statistiche e, da sole, non bastano a fornire una misura completa dell'incertezza associata a una stima. In particolare, consideriamo l'**errore standard**, spesso utilizzato per quantificare l'incertezza di una stima campionaria. Ma è davvero sempre una misura affidabile? Possiamo applicarlo indistintamente in qualsiasi contesto?  

Il caso della bilancia difettosa, descritto in un [post](https://statmodeling.stat.columbia.edu/2023/01/06/god-is-in-every-leaf-of-every-tree-bathroom-scale-edition/) di Andrew Gelman, ci offre un esempio concreto che mette in discussione questa assunzione e ci invita a riflettere più a fondo su cosa significhi davvero misurare l'incertezza.

### L’errore standard è sempre una buona misura di incertezza?

L'errore standard diminuisce all'aumentare del numero di misurazioni, secondo la legge $1/\sqrt{n}$. In teoria, questo significa che possiamo ottenere stime sempre più precise semplicemente raccogliendo più dati. Tuttavia, questo ragionamento è valido solo se le misurazioni sono affette esclusivamente da **variabilità casuale** e non da **bias sistematici**.

Nel caso della bilancia difettosa, il problema non è solo la variabilità delle misurazioni: il peso registrato è influenzato da errori sistematici, come uno zero instabile o una taratura inaffidabile. Di conseguenza, anche se il valore oscilla in modo apparentemente casuale, queste fluttuazioni sono in realtà legate a un errore strutturale che non si annulla aumentando il numero di osservazioni.  

### Bias e deviazione standard: una misura più realistica dell’incertezza

Dal punto di vista statistico, quando c'è **bias**, la **deviazione standard** delle misurazioni è una rappresentazione più realistica dell'incertezza rispetto all'errore standard. L’incertezza effettiva **non** diminuisce semplicemente raccogliendo più dati: se ignoriamo il bias, rischiamo di ottenere stime apparentemente molto precise ma sostanzialmente errate. Questo fenomeno è stato descritto nel **paradosso dei grandi dati** di Xiao-Li Meng: se non si tiene conto del bias, si possono ottenere intervalli di confidenza estremamente stretti, che però danno un'illusione di accuratezza senza alcuna garanzia di affidabilità.

### Conclusione: Variabilità casuale o bias sistematico?

Quando valutiamo l’incertezza di una misurazione, dobbiamo chiederci: **la principale fonte di errore è la variabilità casuale o il bias sistematico?**  

Se l’incertezza è dovuta alla variabilità casuale, l’errore standard è una misura adeguata e la precisione della stima può essere migliorata aumentando la dimensione del campione.  
Se invece è presente un bias, **nessuna quantità di dati potrà compensarlo**, e sarà necessario un approccio diverso, come la calibrazione dello strumento o una modellizzazione del bias stesso, per ottenere una stima realmente affidabile.
