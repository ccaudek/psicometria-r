...
::: {.epigraph}
> “The [Metropolis] algorithm... has the remarkable property that, *instead of considering all possible states of the system, it concentrates on those states which have the greatest probability*.”
>
> -- **Marshall N. Rosenbluth**, raccontando il lavoro del team di Metropolis, Rosenbluth, Rosenbluth, Teller e Teller (1953)
:::
...
# Dati per il grafico (tidy senza reshape2)
...
# Mappatura Okabe–Ito (da _common.R)
colori_okabe <- c(
  "Prior: Beta(4, 6)"       = unname(okabe_ito["sky"]),
  "Posterior: Beta(18, 92)" = unname(okabe_ito["vermillion"])
)
...
```{r}
colori_okabe <- c(
  "Istogramma"  = unname(okabe_ito["sky"]),
  "Beta(18,92)" = unname(okabe_ito["vermillion"])
)
...
p <- tibble(Theta = post_burnin_samples) |>
  ...
  scale_fill_manual(name = "Distribuzione",
                    values = colori_okabe,
                    breaks = names(colori_okabe)) +
  scale_color_manual(name = "Distribuzione",
                     values = colori_okabe,
                     breaks = names(colori_okabe)) +
  guides(
    # Mostra un rettangolo per "Istogramma" (fill) e una linea per "Beta"
...
```{r}
# ── Colori Okabe–Ito (già definiti in _common.R) ─────────────────────
colori_okabe <- c(
  "MCMC"      = unname(okabe_ito["sky"]),         # #56B4E9
  "Analitico" = unname(okabe_ito["bluishgreen"])  # #009E73
)
