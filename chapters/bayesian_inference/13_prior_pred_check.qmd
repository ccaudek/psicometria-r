...
<p class="capo">
  ...
  ell'analisi bayesiana, la specificazione della *distribuzione a priori* (*prior*) rappresenta un passaggio cruciale e delicato. Attraverso la prior, formalizziamo matematicamente le nostre convinzioni o aspettative riguardo ai parametri del modello *prima di osservare i dati*. Ma come possiamo valutare se la nostra scelta è appropriata?  
</p>
...
ppc_bars(y = rep(0, n_studenti), yrep = yrep[1:200, ]) +
  ggtitle("Prior Predictive Check (modello binomiale, intercetta in scala logit)")
```
...
tibble(mu = rowMeans(y_rep)) |>
  ggplot(aes(x = mu)) +
  geom_histogram(bins = 30, fill = rinascimentale_primaria["seppia_scuro"], color="white") +
  labs(title = "Distribuzione predittiva precedente (Stan, binomiale, intercetta in scala logit)",
       x = "Media delle risposte corrette (su 10 item)", 
       y = "Frequenza")
```
...
bind_rows(
  ...
  ggplot(aes(x = y, fill = scenario)) +
  geom_density(alpha = 0.35, fill = rinascimentale_primaria["seppia_scuro"], color="white") +
...
Il controllo predittivo a priori costituisce una fase fondamentale nell'analisi bayesiana, rappresentando il punto di incontro tra la formalizzazione matematica del modello e la conoscenza sostantiva del fenomeno indagato. Questa pratica trascende la mera verifica tecnica, configurandosi come un autentico esercizio epistemologico volto a valutare la coerenza tra le aspettative teoriche e le implicazioni quantitative del modello proposto.

Nella prassi della ricerca empirica, risulta essenziale documentare con precisione:

-   le procedure di simulazione dei dati generate dalle distribuzioni a priori specificate,
-   le eventuali discordanze emerse tra i dati simulati e le aspettative teoriche di riferimento,
-   le modifiche apportate alle distribuzioni a priori in risposta a tali incongruenze,
-   i risultati definitivi ottenuti a seguito della ricalibrazione del modello.

Questa documentazione non rappresenta un mero adempimento metodologico, bensì un elemento cruciale per garantire la trasparenza e la riproducibilità del processo analitico. Si consideri, a titolo esemplificativo, un test psicometrico composto da 10 item: l'adozione di una distribuzione Beta(2,2) riflette una sostanziale incertezza a priori, generando simulazioni caratterizzate da un'ampia variabilità di punteggi; al contrario, una Beta(10,2) esprime la convinzione che i partecipanti posseggano generalmente competenze elevate, concentrando le previsioni sui punteggi più alti.

In psicologia, disciplina caratterizzata da costrutti teorici spesso dai contorni sfumati, questo approccio metodologico assume particolare rilevanza. A differenza dei framework analitici che iniziano l'indagine solo successivamente alla raccolta dati, l'impianto bayesiano promuove una riflessione anticipata sulle ipotesi modellistiche e sulle loro conseguenze predittive, favorendo una più matura consapevolezza epistemologica.

Il controllo predittivo a priori si rivela dunque uno strumento diagnostico di notevole efficacia:

-   segnala eventuali discrepanze tra le assunzioni del modello e la realtà empirica di riferimento,
-   orienta la riparametrizzazione delle distribuzioni a priori in coerenza con la conoscenza pregressa,
-   consolida la validità ecologica delle analisi successive.

Rendendo esplicite le implicazioni connesse alle scelte modellistiche, questa pratica favorisce lo sviluppo di analisi maggiormente consapevoli e robuste. In definitiva, il controllo predittivo a priori non rappresenta un semplice passaggio tecnico, bensì un momento essenziale per costruire modelli che riflettano autenticamente la complessità dei fenomeni psicologici oggetto di indagine.
