...
Il metodo su griglia è un approccio semplice e didattico: discretizziamo lo spazio dei possibili valori di $\theta$ e calcoliamo la distribuzione a posteriori passo per passo.

**Passaggi:**

1. definiamo una griglia di valori plausibili per $\theta$ (da 0 a 1),
2. calcoliamo la verosimiglianza per ogni valore di $\theta$,
3. moltiplichiamo verosimiglianza e priori,
4. normalizziamo i valori ottenuti in modo che la somma sia 1.
...
ggplot(data.frame(theta, prob = priori_unif), aes(x = theta, y = prob)) +
  geom_col(width = 0.08, fill = "#5d5349") +
