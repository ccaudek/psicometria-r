...
La **Legge dei Grandi Numeri** (LLN) rappresenta un fondamento della probabilità: se $\bar{X}_n$ è la media di $n$ osservazioni iid con valore atteso $\mu$, allora $\bar{X}_n \to \mu$ quando $n\to\infty$ (si veda il @sec-prob-interpretation). Informalmente, più individui valutiamo, più la media campionaria si avvicina stabilmente al vero parametro.

Esistono due versioni principali:

1. **Legge Forte**: $\bar{X}_n$ converge quasi certamente a $\mu$.
2. **Legge Debole**: $\bar{X}_n$ converge a $\mu$ in probabilità, cioè per ogni $\varepsilon>0$,
   $$
   \Pr\bigl(| \bar{X}_n - \mu| > \varepsilon\bigr) \to 0 \quad \text{al crescere di }n.
   $$

In psicologia, ciò significa, per esempio, che se vogliamo conoscere con buona accuratezza la proporzione di individui che mostra un certo fenomeno (o la media di un test cognitivo), dobbiamo raccogliere un *numero sufficiente* di misurazioni affinché la media del campione sia vicina alla media “vera” della popolazione.
