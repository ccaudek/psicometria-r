...
<p class="capo">
  <span class="capo-initial" data-bg="7">I</span>n questo capitolo ci concentreremo sull'analisi dei dati numerici. In particolare, esamineremo le distribuzioni di frequenza e i quantili, insieme alle tecniche di visualizzazione più comuni, come l'istogramma, l'istogramma smussato e il box-plot. Tratteremo sia gli aspetti computazionali che quelli interpretativi di queste misure, fornendo strumenti utili non solo per una comprensione personale, ma anche per la comunicazione efficace dei risultati, in particolare con chi utilizza questi dati per prendere decisioni pratiche nel mondo reale.
</p>
...
ggplot(tmp, aes(x = height, y = density)) +
  ...
  labs(
    title = "Distribuzione dell'altezza nei maschi",
...
heights |> 
  ...
  labs(
    title = "Distribuzione stimata dell'altezza (maschi)",
    x = "Altezza (in pollici)",
    y = "Densità"
  )
```
...
palette_group <- setNames(
  c(okabe_ito["blue"], okabe_ito["vermillion"]),
  c("ctl", "mdd")
)

# Assicura livelli ordinati
df <- df |> mutate(group = factor(group, levels = c("ctl", "mdd")))

# Helper: mean ± SE
mean_se <- function(x, mult = 1) {
  m  <- mean(x, na.rm = TRUE)
  se <- stats::sd(x, na.rm = TRUE) / sqrt(sum(!is.na(x)))
  data.frame(x = m, xmin = m - mult*se, xmax = m + mult*se)
}

ggplot(df, aes(y = group, x = bdi)) +
  # "Barre" orizzontali come media (geom_col con stat='summary')
  stat_summary(
    fun = mean,
    geom = "col",
    aes(fill = group),
  ...
  scale_fill_manual(values = palette_group, drop = FALSE) +
  labs(
    x = "BDI-II",
    y = "Gruppo",
    title = "(A) Media con errore standard (orizzontale)"
  ) +
  theme(plot.title = element_text(hjust = 0.5))

```

Il classico bar plot si limita a mostrare la media e l’errore standard. È molto diffuso, ma rischia di essere fuorviante perché non rivela la forma della distribuzione, eventuali asimmetrie o la presenza di valori anomali.
...
ggplot(df, aes(y = group, x = bdi, fill = group)) +
  geom_boxplot(width = 0.5, outlier.size = 0.8) +
  scale_fill_manual(values = palette_group, drop = FALSE) +
  labs(
    x = "BDI-II",
    y = "Gruppo",
    title = "(B) Boxplot (orizzontale)"
  ) +
  theme(plot.title = element_text(hjust = 0.5))
```

Il boxplot è un passo avanti: mostra mediana, quartili e valori estremi. Tuttavia rimane una rappresentazione sintetica che può non rendere pienamente l’idea della distribuzione complessiva.
...
ggplot(df, aes(y = group, x = bdi, fill = group)) +
  ...
  labs(
    x = "BDI-II",
    y = "Gruppo",
    title = "(C) Violin plot (orizzontale)"
  ) +
  theme(plot.title = element_text(hjust = 0.5))
```

Il violin plot aggiunge un’informazione preziosa: la curva di densità, che consente di cogliere la forma della distribuzione (per esempio unimodale o bimodale). Manca però la rappresentazione puntuale dei dati osservati.
...
library(ggrain)  # package dedicato per Raincloud plots

ggplot(df, aes(x = group, y = bdi, fill = group)) +
  geom_rain(
    rain.side = "l",          # densità e punti sul lato sinistro
    alpha = 0.6,              # trasparenza
    boxplot.args = list(color = "black", outlier.shape = NA),
    boxplot.args.pos = list(position = position_nudge(x = 0.1), width = 0.05),
    violin.args = list(alpha = 0.5),
    violin.args.pos = list(side = "r", width = 0.6, position = position_nudge(x = 0.2)),
    point.args = list(size = 1, color = "black", alpha = 0.7),
    point.args.pos = list(position = position_jitter(width = 0.02, height = 0))
  ) +
  coord_flip() +  # orienta orizzontalmente: categorie sull'asse Y, valori sull'asse X
  scale_fill_manual(values = palette_group, drop = FALSE) +
  labs(
    x = "BDI-II",
    y = "Gruppo",
    title = "(D) Raincloud plot (orizzontale, con ggrain)"
  ) +
  theme(plot.title = element_text(hjust = 0.5), legend.position = "none")
```

Il raincloud plot integra tre elementi: una *nuvola* (densità, simile a un violin plot), una *pioggia* di punti che mostra i dati grezzi e un *boxplot orizzontale* che sintetizza le statistiche principali. Questa combinazione lo rende particolarmente utile nell’analisi esplorativa, perché permette di individuare a colpo d’occhio skewness, cluster e outliers, soprattutto nel confronto tra gruppi. Diversi studi, tra cui @weissgerber2015beyond e @weissgerber2019reveal, hanno dimostrato che visualizzazioni ricche come il *raincloud plot* riducono il rischio di interpretazioni errate.
