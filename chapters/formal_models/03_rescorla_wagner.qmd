...
A questa componente di apprendimento si aggiunge un livello decisionale: le scelte non sono deterministiche, ma bilanciano esplorazione e sfruttamento. Nei compiti binari (due opzioni) la trasformazione dei valori appresi in probabilità di scelta è ben descritta dalla funzione logistica (equivalente alla Softmax a due opzioni):

$$
P(\text{scegliere B}) = \text{inv\_logit}\!\big(\tau \,[Q_t(B)-Q_t(A)]\big),
$$
dove $\tau>0$ (inverse temperature) regola quanto le differenze di valore si traducano in scelte deterministiche (τ alta) o esplorative (τ bassa).
...
ggplot(df, aes(x = dq, y = prob_B, color = tau)) +
  ...
  labs(
    ...
    title = "Effetto di τ sul compromesso esplorazione–sfruttamento"
  )
```
...
ggplot(sim_long, aes(x = trial, y = Q, color = stimolo)) +
  ...
  labs(
    x = "Trial", y = "Valore Q",
    color = NULL,
    title = "Traiettorie dei valori associativi (Q) con reversal a metà"
  ) +
...
Il modello di *Rescorla–Wagner con regola di scelta logistica* arricchisce la semplice dinamica di aggiornamento dei valori introducendo un *livello decisionale* che traduce i valori appresi (*Q-values*) in probabilità di scelta. In questo modo possiamo distinguere due componenti fondamentali del comportamento:

1. **Apprendimento dagli esiti** (*learning*), governato dal parametro `lr`, che determina la rapidità con cui il soggetto aggiorna le proprie aspettative in base al *prediction error* (quanto l’esito è diverso da ciò che ci si aspettava).

   * `lr` alto → adattamento veloce, ma anche maggiore sensibilità al rumore;
   * `lr` basso → apprendimento lento, più stabile ma meno flessibile ai cambiamenti.

2. **Politica decisionale** (*decision policy*), rappresentata dal parametro `tau`, che regola il grado di coerenza delle scelte con i valori stimati.

   * `tau` alto → scelte deterministiche e coerenti con l’opzione migliore;
   * `tau` basso → comportamento più esplorativo o meno consistente.

Questa distinzione è particolarmente rilevante negli studi clinici. Nel nostro lavoro sull’anoressia nervosa e il *Probabilistic Reversal Learning* (PRL), ad esempio, è emerso che i deficit non sono globali, ma *specifici per il contesto alimentare*: le pazienti aggiornano più lentamente le loro aspettative (`lr` ridotto) quando sono coinvolti stimoli *food*, mentre si comportano come i controlli in condizioni neutrali [@colpizzi2025food].

Il modello RW + logit permette quindi di andare oltre la semplice prestazione media:

* da un lato, evidenzia differenze nei *meccanismi di apprendimento* (`lr`) tra condizioni,
* dall’altro, consente di verificare se tali differenze si accompagnano a uno *stile decisionale* diverso (`tau`).

Per uno psicologo questo comporta due vantaggi:

* **Chiarezza teorica:** si possono formulare ipotesi distinte sui processi di aggiornamento e sulle strategie decisionali, evitando interpretazioni riduttive basate solo sull’accuratezza.
* **Rilevanza clinica:** distinguere tra un problema di apprendimento e uno di scelta ha implicazioni dirette per interventi mirati, ad esempio potenziando la sensibilità al feedback o modulando l’equilibrio tra esplorazione e sfruttamento.

In conclusione, il modello RW + logit non è soltanto uno strumento computazionale: è un *ponte tra il comportamento osservato e i processi psicologici sottostanti*, utile per isolare deficit specifici di contesto e per comprendere come le strategie decisionali possano variare nei diversi individui e gruppi clinici.

Operativamente, la versione gerarchica multi-soggetto del modello è spesso preferibile: stabilizza le stime individuali, consente confronti tra gruppi/condizioni e integra naturalmente la variabilità inter-individuale.
