...
::: {.epigraph}
> “Generations of writers opined vaguely that ‘simple hypotheses are more plausible’ without giving any logical reason for it. We suggest that this should be turned around: we should say rather that ‘more plausible hypotheses tend to be simpler’.”
>
...
<p class="capo">
  <span class="capo-initial" data-bg="8">N</span>
...
Il principio fondamentale della modellazione bayesiana è che la qualità di un modello si valuta in base alla sua capacità di fare previsioni probabilistiche robuste. Queste previsioni, contenute nella distribuzione predittiva a posteriori $p(\tilde{y} \mid y)$, incorporano in modo naturale l'incertezza sui parametri, offrendo una visione più completa e realistica della relazione tra il modello e i dati.

La bussola che guida questa valutazione è l'*Expected Log Predictive Density* (ELPD), una misura che stima quanto bene il modello sia in grado di prevedere dati mai visti prima. A differenza delle misure calcolate sugli stessi dati usati per l'adattamento, che rischiano di essere troppo ottimistiche, l'ELPD offre una stima onesta della capacità di generalizzazione. Dal punto di vista teorico, massimizzare l'ELPD equivale ad avvicinarsi il più possibile alla vera distribuzione dei dati, minimizzando la divergenza di Kullback-Leibler. In altre parole, un ELPD più alto segnala un modello le cui previsioni sono più vicine alla realtà sottostante.

Nella pratica, lo strumento raccomandato per stimare l’ELPD è il PSIS-LOO, efficiente e ricco di informazioni grazie alle sue diagnostiche, come gli indici Pareto-k. Qualora questi ultimi segnalassero delle criticità, è possibile ricorrere ad altre tecniche altrettanto valide, come la convalida incrociata K-fold o l'indice LOO per cluster, particolarmente utile in presenza di dati gerarchici o dipendenti.

Accanto al LOO, il WAIC rappresenta un altro criterio solido e completamente bayesiano, spesso in linea con i risultati del LOO e utile come controllo aggiuntivo. Al contrario, i criteri AIC e BIC, sebbene intuitivi e computazionalmente leggeri, si basano su stime puntuali e approssimazioni asintotiche, risultando meno adatti in contesti con piccoli campioni o modelli gerarchici.

Quando si confrontano i modelli, è importante riportare non solo il valore di ELPD-LOO, ma anche la differenza tra i modelli (ΔELPD) e il suo errore standard, le diagnostiche Pareto-k e una riflessione sulla parsimonia e sull'interpretabilità del modello preferito. È necessario evitare valutazioni puramente *in-sample*, come il MSE sul training set, e prestare attenzione all'indipendenza delle osservazioni durante la convalida, al fine di giungere a conclusioni valide.

In definitiva, la scelta di un modello non dovrebbe essere ridotta a una scelta meccanica del modello con il punteggio più alto. Differenze minime nell'ELPD, specialmente se associate a un ampio errore standard, potrebbero non essere utili dal punto di vista pratico. Spesso, un modello leggermente meno performante, ma più parsimonioso o meglio allineato alla teoria, può rivelarsi la scelta migliore. L'obiettivo è trovare un equilibrio tra la capacità predittiva e la coerenza teorica, ricordando che lo scopo non è solo prevedere, ma anche comprendere.

In sintesi, valutare un modello significa verificarne la capacità di generalizzare attraverso stime fuori campione, come l'ELPD, utilizzando strumenti come il PSIS-LOO. Il confronto deve sempre tenere in considerazione l'incertezza delle stime, la struttura dei dati e il contesto teorico. Alla fine, un buon modello non è solo quello che prevede meglio, ma anche quello che spiega in modo convincente il fenomeno studiato.
