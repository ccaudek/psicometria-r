...
In questo capitolo descriveremo il concetto di **entropia**, una misura fondamentale della teoria dell'informazione. L'entropia ci permette di **quantificare l'incertezza** associata a una distribuzione di probabilità e, di conseguenza, la **quantità di informazione** che un evento ci fornisce. Prima di addentrarci nei calcoli, iniziamo con un’idea intuitiva: più un evento è imprevedibile, maggiore è l’entropia.

::: {.callout-note title="Idea chiave"}
L’entropia è una misura dell’**incertezza media** di un sistema:

* eventi **tutti ugualmente probabili** → entropia massima.
* eventi **molto sbilanciati** nelle probabilità → entropia minore.
:::

**Esempio intuitivo**:

* Sacchetto con palline tutte dello stesso colore → sappiamo già l’esito → **entropia = 0**.
* Sacchetto con palline di più colori in pari quantità → ogni estrazione è incerta → **entropia massima**.
...
Ora introduciamo un ingrediente in più: **la probabilità dell’evento**. Se due eventi non sono ugualmente probabili, la “quantità di informazione” che trasmettono cambia:

* evento molto probabile → poca sorpresa → poca informazione;
* evento raro → molta sorpresa → più informazione.

Shannon definì l’**informazione** (o sorpresa) di un evento $x$ come:
...
| Probabilità $p(x)$ | Sorpresa $h(x)$ [bit] |
|----------------------|------------------------|
| 0.5                  | 1.00                   |
| 0.25                 | 2.00                   |
| 0.10                 | 3.32                   |

Più l’evento è raro, più alto è il numero di bit.


## Entropia come Media dell'Informazione di Shannon

Finora abbiamo parlato di sorpresa di un singolo evento. Ma in un sistema, vogliamo misurare la **sorpresa media** considerando *tutti* i possibili esiti → **questa è l’entropia**.

Matematicamente, l’**entropia** è la **media pesata** della sorpresa di tutti i possibili esiti di una variabile casuale $X$:
...
- **Entropia alta** → distribuzione uniforme → alta imprevedibilità.  
- **Entropia bassa** → distribuzione sbilanciata → prevedibilità maggiore.
...
::: callout-note
**Interpretazione rapida:** l’entropia è il numero medio di bit necessari per descrivere un’osservazione di $X$.
:::


## Interpretazione dell'entropia

Ora diamo significato al numero che otteniamo. L’entropia espressa in bit è la quantità di informazione che serve per distinguere tra $m = 2^{H(X)}$ alternative ugualmente probabili.
...
::: {#exercise-entropy-interpretation .callout .exercise collapse="true" collapsed="true"}
...
- *Additività dell'entropia per eventi indipendenti:* l'entropia è additiva nel caso di eventi indipendenti. Ciò significa che, se si verificano due o più eventi indipendenti, l'entropia totale della loro combinazione è pari alla somma delle entropie di ciascun evento considerato singolarmente. Questa proprietà deriva dall'additività dei logaritmi che permette di sommare le entropie individuali per ottenere l'entropia complessiva.
