...
Quando studiamo un fenomeno psicologico, spesso ci chiediamo: *esiste una relazione tra due variabili?* Ad esempio: l’ansia influisce sul rendimento accademico? Il tempo di studio è associato a risultati migliori nei test? Un *modello statistico* è una rappresentazione semplificata della realtà che ci aiuta a rispondere a domande come queste. In particolare, ci dice cosa succede, *in media*, a una variabile (che chiamiamo $Y$, o *variabile dipendente*) quando un’altra variabile ($X$, o *variabile indipendente*) cambia.  

### La relazione tra $X$ e $Y$ 

Il modello di *regressione lineare bivariato* è il più semplice strumento per descrivere una relazione tra due variabili. L’idea di base è che, se $X$ e $Y$ sono associate, possiamo rappresentare questa relazione con una *retta* che esprime come $Y$ tende a variare al variare di $X$.  

Il modello descrive la relazione tra $X$ e $Y$ attraverso un'equazione lineare:
...
Geometricamente, questa equazione corrisponde a una *retta di regressione*, che rappresenta la migliore approssimazione lineare della relazione tra $X$ e $Y$ nei dati osservati.  

Nella realtà, raramente tutti i punti dati giacciono perfettamente sulla retta: ci sono sempre variazioni individuali. La regressione lineare non pretende di prevedere *esattamente* il valore di $Y$ per ogni osservazione, ma ci dice cosa ci aspettiamo *in media* in una popolazione.  

Ad esempio, se troviamo che $b = 2$, significa che, *in media*, ogni aumento di 1 punto in $X$ è associato a un aumento di 2 punti in $Y$. Questo non implica che ogni singolo caso segua questa regola, ma che, osservando molti dati, la tendenza generale è ben descritta da questa relazione lineare.  

### A cosa serve la regressione?

Secondo @gelman2021regression, la regressione lineare ha quattro applicazioni principali:
...
**Avvertenza cruciale:** in tutti questi contesti, è *essenziale* che il modello includa tutte le variabili rilevanti per la variabile di interesse. Ad esempio, in uno studio sull’efficacia di una psicoterapia per la depressione, fattori come età, condizioni di salute preesistenti e supporto sociale possono influenzare significativamente i risultati e devono essere integrati nell’analisi. L'omissione di tali variabili può distorcere le relazioni stimate tra predittori e risultato -- un problema noto come *errore di specificazione*. L’accuratezza dei modelli di regressione dipende quindi dalla completezza del modello.  

### Stima dei parametri: due prospettive a confronto

La stima dei parametri del modello di regressione lineare – inclinazione ($a$), intercetta ($b$) e deviazione standard degli scarti tra dati osservati e valori stimati ($\sigma$) – può essere ottenuta seguendo due approcci distinti: il metodo frequentista (o classico) e l'approccio bayesiano.


#### L’approccio frequentista (minimi quadrati e massima verosimiglianza)  

Nell’approccio frequentista, i coefficienti $a$ (inclinazione) e $b$ (intercetta) vengono stimati individuando la retta che minimizza la *somma dei quadrati degli errori* (*SSE*), definiti come:  

$$
e_i = y_i - (a + b x_i), \quad \text{con} \quad SSE = \sum_{i=1}^n e_i^2.
$$  

Se si assume che gli errori siano distribuiti normalmente con media zero e varianza costante ($e_i \sim N(0, \sigma^2$), la stima dei *minimi quadrati ordinari (OLS)* coincide con quella di *massima verosimiglianza (MLE)*. Questo metodo fornisce stime puntuali univoche, individuando la retta che meglio si adatta ai dati osservati.  

L’incertezza associata alle stime viene quantificata mediante *intervalli di confidenza* (es. al 95%). Un intervallo di confidenza al 95% per un parametro indica che, se l’esperimento fosse ripetuto infinite volte sotto le stesse condizioni, nel 95% dei casi il vero valore del parametro cadrebbe all’interno di tale intervallo.  

La forza di questa impostazione sta nella sua semplicità operativa, ma la sua interpretazione probabilistica rimane ancorata a una logica di frequenze a lungo termine e non integra informazioni pregresse. Inoltre, richiede assunzioni stringenti: normalità, omoschedasticità e indipendenza degli errori.  
...
3. **Interpretazione del posterior**  
...
La scelta tra metodi frequentisti e bayesiani dipende dal contesto di ricerca e dagli obiettivi analitici.  

- *L'approccio frequentista* rimane ampiamente utilizzato in contesti con dati abbondanti (dove l'effetto dei *prior* diventa trascurabile), standard consolidati (es., analisi replicative o applicazioni regolatorie) e esigenze di semplicità computazionale. Ad esempio, un ricercatore che replichi uno studio classico sull'efficacia di una terapia cognitiva ($X$) nel ridurre l'ansia ($Y$) potrebbe optare per i minimi quadrati, garantendo comparabilità con la letteratura esistente.  

- *L'approccio bayesiano* è invece preferibile quando esistono conoscenze pregresse rilevanti (da meta-analisi o evidenza teorica), si richiede un'interpretazione probabilistica intuitiva (es., intervalli di credibilità) e quando i dati sono scarsi o costosi (i *prior* stabilizzano le stime). Un caso emblematico è la sintesi di evidenze eterogenee: un'analisi bayesiana che integri studi precedenti con nuovi dati offre una stima aggiornata e coerente, con una quantificazione trasparente dell'incertezza.  

In sintesi, nessun paradigma è universalmente superiore. La decisione deve considerare la natura del problema, la disponibilità di informazioni a priori e le esigenze interpretative. La crescente accessibilità di strumenti computazionali sta comunque rendendo il bayesiano una scelta sempre più praticabile anche per problemi tradizionali.  

### Valutare la bontà del modello

La qualità dell’adattamento si apprezza osservando l’indice di determinazione $R^2$, che indica quanta parte della varianza di $y$ viene spiegata dal modello, e analizzando i residui: eventuali pattern sistematici nei residui possono segnalare che la struttura scelta non coglie tutte le caratteristiche dei dati o che esistono violazioni delle ipotesi (linearità, omoscedasticità, normalità degli errori). Un esame congiunto di $R^2$ e residui aiuta a diagnosticare e, se necessario, migliorare il modello.

### Tipologie di regressione  

La regressione si articola in tre livelli di complessità:  
...
Il caso bivariato, pur essendo il più semplice, è raramente sufficiente per descrivere fenomeni complessi, soprattutto in psicologia, dove molteplici fattori influenzano le variabili di interesse. Tuttavia, lo analizzeremo in dettaglio perché permette di comprendere la logica dell'analisi di regressione in un contesto semplificato, privo di complicazioni matematiche. I concetti appresi possono poi essere estesi a modelli più complessi, i quali richiederanno calcoli più articolati e un'interpretazione più sofisticata dei parametri.

## La Predizione dell'Intelligenza

Questo capitolo offre una trattazione pratica del modello di regressione lineare semplice, sviluppando le competenze necessarie per affrontare successivamente modelli più complessi. A tale scopo, utilizzeremo un dataset reale: i dati `kidiq` tratti dal *National Longitudinal Survey of Youth* [@gelman2021regression].  

Questi dati riguardano un campione di donne americane e i loro figli, con particolare attenzione a due variabili fondamentali per la nostra analisi: il punteggio cognitivo del bambino (`kid_score`) e il quoziente intellettivo della madre (`mom_iq`). L'obiettivo principale consiste nell'esaminare se - e in che misura - l'intelligenza materna possa essere considerata un fattore predittivo delle capacità cognitive del bambino.  

Per condurre questa indagine, procediamo con l'importazione dei dati nell'ambiente R.
