...
::: {.epigraph}
> “It is the purpose of the mathematician to be dealing with the relationship between quantities, so that from one the other can be estimated.”
>
> -- **Carl Friedrich Gauss **, sul metodo dei minimi quadrati
:::
...
ggplot(kidiq, aes(x = mom_iq, y = kid_score)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = okabe_ito["sky"]) +
  annotate(
...
ggplot(kidiq, aes(x = xd, y = kid_score)) +
  ...
  geom_abline(intercept = a, slope = b, color = okabe_ito["sky"]) +  # Retta di regressione
  labs(
...
ggplot(dat, aes(x = b_grid, y = sse_vals)) +
  geom_line(color = okabe_ito["sky"], linewidth = 1) +  
  annotate(
...
ggplot(data.frame(b_hat = b_hat), aes(x = b_hat)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 50,
                 fill = okabe_ito["sky"],        # istogramma
                 color = "white") +
  stat_function(fun = dnorm,
                args = list(mean = mean_b_hat, sd = sd_b_hat),
                color = okabe_ito["vermillion"], # curva normale
                linewidth = 1) +
  labs(
