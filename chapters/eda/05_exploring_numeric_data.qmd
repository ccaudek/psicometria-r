...
#| out-width: 100%  
#| 
# --- Palette Okabe–Ito (già definita in _common.R); qui selezioniamo 2 colori ---
...
df <- df |>
  dplyr::mutate(group = factor(group, levels = c("ctl", "mdd")))

# Helper: media ± SE (errore standard), non ± SD
...
raincloud_plot <- ggplot(df, aes(x = group, y = bdi, fill = group)) +
  ...
  scale_fill_manual(values = palette_group, drop = FALSE) +
  scale_color_manual(values = palette_group, drop = FALSE) +
  labs(
    x = "Gruppo",
    y = "BDI-II",
