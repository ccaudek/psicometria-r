...
::: {.epigraph}
> “Stan is more than a tool; it's a *practical revolution in Bayesian computation*. It turns the impossible into the tractable and the complex into the computable.”
>
> -- Adattata dalla documentazione ufficiale di Stan.
:::

## Introduzione {.unnumbered .unlisted}

::: {.lead}
[Stan](https://mc-stan.org/) è un linguaggio di programmazione probabilistica (*Probabilistic Programming Language*, PPL) progettato per l'inferenza bayesiana in modelli statistici complessi. Una delle sue caratteristiche distintive è l'implementazione del *No-U-Turn Sampler* (NUTS), un algoritmo avanzato che ottimizza il metodo Hamiltonian Monte Carlo (HMC). Rispetto al tradizionale algoritmo di Metropolis (@sec-mcmc-metropolis), NUTS offre una convergenza più rapida alla distribuzione a posteriori, riducendo drasticamente il numero di iterazioni necessarie, specialmente in modelli ad alta dimensionalità. Pur producendo risultati equivalenti dal punto di vista teorico, NUTS rappresenta un significativo passo avanti in termini di efficienza computazionale.
:::
...
La programmazione probabilistica integra i principi della statistica bayesiana con i linguaggi di programmazione, semplificando la costruzione di modelli complessi (si veda @nicenboim2025introduction per una trattazione approfondita). In un PPL come Stan, il ricercatore deve solo specificare:

1. le distribuzioni a priori dei parametri,  
...
Ogni programma Stan è organizzato in blocchi distinti, che corrispondono a funzioni precise. Nel blocco `data` dichiariamo le variabili osservate che passiamo dall’esterno; in `parameters` indichiamo le quantità ignote che vogliamo stimare; nel blocco `model` specifichiamo le assunzioni probabilistiche — cioè le distribuzioni a priori e la verosimiglianza; infine, in `generated quantities` possiamo calcolare misure derivate, come predizioni o log-likelihood, che non influiscono sulla stima ma sono utili per l’analisi successiva. Questa architettura modulare rende Stan intuitivo e adattabile a un'ampia gamma di applicazioni statistiche.
...
tibble(y = y_cont) |>
  ...
  labs(title = "Distribuzione simulata dei punteggi QI",
...
p <- bayesplot::mcmc_areas(fit2$draws("mu"), prob = 0.94) +
  ggtitle("Posterior di mu: ETI (area) e HDI (linee) al 94%") +
...
ggplot(data.frame(theta = theta_draws), aes(x = theta)) +
  ...
  labs(title = "Posterior asimmetrica (Beta): ETI vs HDI (94%)",
...
In definitiva, imparare Stan non significa solo padroneggiare un software, ma acquisire un nuovo modo di pensare ai dati psicologici. Significa ragionare in termini di incertezza, mettere alla prova le proprie ipotesi e rendere trasparenti i propri modelli. È uno strumento che, una volta compreso, cambia il modo di fare ricerca.
